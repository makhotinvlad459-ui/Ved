...
def process_cz(session_id: str):
    ...
    try:
        with SessionLocal() as db:
            ...
            session.status = "completed"
            session.result_file = output_dir
            session.total_items = len(groups)
            session.processed_items = len(files)
            db.commit()
            
            print(f"✅ ЧЗ {session_id} завершён, создано {len(files)} файлов")
            return {"status": "completed", "session_id": session_id, "files": len(files)}
